...
本文档介绍了Istio：一个用来连接、管理和保护微服务的开放平台。使用Istio可以让已部署服务的负载均衡、服务间认证、监控等来的更轻松，而不需要改动服务的代码。想要为服务增加对Istio的支持，您只需要在环境中部署一个特殊的边车（sidecar），使用Istio控制平面功能配置和管理代理，拦截微服务之间的所有网络通信。
...
Istio解决了开发人员和运维人员在从单体应用程序向分布式微服务架构的转型过程中面临的诸多挑战。术语**服务网格（Service Mesh）**通常用于描述构成这些应用程序的微服务网络以及它们之间的交互。随着服务网格的规模和复杂性的增长，它变得越来越难以理解和管理。它的需求包括服务发现、负载均衡、故障恢复、指标收集和监控以及通常更复杂的运维需求，例如A/B测试、金丝雀发布、限流、访问控制和端到端认证等。
